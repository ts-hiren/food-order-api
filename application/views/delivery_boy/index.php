	...
	<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
		<div class="d-flex">
			<div class="breadcrumb">
				...
				<span class="breadcrumb-item active">Delivery Boy</span>
			</div>
...
<div class="content">
	<div class="card">
		<div class="card-header header-elements-inline">
			<h5 class="card-title">Manage Delivery Boys</h5>
			<button type="button" onclick="openModal('Add Delivery Boy','<?= base_url('delivery-boy/create') ?>','md')" class="btn bg-teal-400 btn-labeled pull-right btn-labeled-left rounded-round"><b><i class="icon-plus3"></i></b> Add Delivery Boy</button>
		</div>
		<div class="card-body">
...
<script type="text/javascript">
	$(function(){
		...
		tbl = $('.my-data-table').DataTable({
			processing: true,
			serverSide: true,
			ajax : '<?= base_url('delivery-boy/list') ?>',
			columns: [
		...
		$('body').on('change','.status-switch',function(e){
            var id = $(this).data('id');
          	$.ajax({
          		url: '<?= base_url('delivery_boy/') ?>'+id+'/status',
          		type: 'post',
	...
	function deleteDelivery_Boy(id)
	{
		swal({
            title: 'Are you sure?',
            ...
            preConfirm: function(){
            	return new Promise(function (resolve) {
	                $.ajax({
	                	url: '<?= base_url('delivery-boy/') ?>'+id+'/delete',
	                	type: 'get',
	                	dataType: 'json',
	                	success: function(a){
                			resolve();
